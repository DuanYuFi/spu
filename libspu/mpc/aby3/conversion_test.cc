...
TEST_P(ConversionTest, B2ATest) {
  ...
  utils::simulate(npc, [&](const std::shared_ptr<yacl::link::Context>& lctx) {
    ...
    ArrayRef bshares(makeType<Pub2kTy>(FM64), 80000);
    auto _bshares = ArrayView<uint64_t>(bshares);

    for (size_t i = 0; i < 10000; i++) {
      for (size_t j = 0; j < 8; j++) {
        _bshares[i * 8 + j] = (public_bits[i] >> j) & 1;
      }
    }

    bshares = obj->call("p2b", bshares);

    if (comm->getRank() == 0) {
      std::cout << bshares.eltype().as<aby3::BShrTy>()->nbits() << std::endl;
    }
    ...
    if (comm->getRank() == 0) {
      std::cout << "cost: " << cost.comm << std::endl;
    }
  });
}
