...
std::vector<ArrayRef> BeaverIo::toShares(const ArrayRef& raw, Visibility vis,
...
std::vector<ArrayRef> BeaverIo::makeBitSecret(const ArrayRef& in) const {
...
ArrayRef BeaverIo::fromShares(const std::vector<ArrayRef>& shares) const {
...
std::unique_ptr<BeaverIo> makeBeaverIo(FieldType field, size_t npc) {
  SPU_ENFORCE(npc == 3U, "beaver is only for 3pc.");
  registerTypes();
  return std::make_unique<BeaverIo>(field, npc);
}
